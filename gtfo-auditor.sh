...
check_base_state() {   # → NOT_INSTALLED / BLOCKED / PASSWD / NOPASSWD
  ...
  # Ist das Kommando laut sudoers irgendwo erlaubt?
  if sudo -n -l -U "$user" "$bin" &>/dev/null; then
      # erlaubt -> NOPASSWD oder PASSWD?
...
check_noexec_flag() {   # NOEXEC‑Flag in sudoers‑Eintrag?
  sudo -n -l -U "$1" "$2" 2>/dev/null | grep -q NOEXEC && echo SET || echo UNSET
}

check_noexec_enforced() {   # ENFORCED / BYPASSED / UNKNOWN
  local user="$1" bin="$2" out
  case "$bin" in
    */vim|*/vi|*/view) out=$(runuser -u "$user" -- sudo -n "$bin" -c ':!whoami' 2>/dev/null);;
    */less)            out=$(echo '!whoami' | runuser -u "$user" -- sudo -n "$bin" 2>/dev/null);;
    */python*|*/python3) out=$(runuser -u "$user" -- sudo -n "$bin" -c 'import os,sys;os.system("whoami")' 2>/dev/null);;
    */perl)            out=$(runuser -u "$user" -- sudo -n "$bin" -e 'system("whoami")' 2>/dev/null);;
    */awk)             out=$(runuser -u "$user" -- sudo -n "$bin" 'BEGIN{system("whoami")}' 2>/dev/null);;
    *)                 echo UNKNOWN; return;;
  esac
